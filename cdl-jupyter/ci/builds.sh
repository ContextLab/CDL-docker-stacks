...
build_default() {
    local image_dir="$GITHUB_WORKSPACE/cdl-jupyter"
    docker build --rm --force-rm \
        -f "$image_dir/Dockerfile" \
        -t "$DOCKER_HUB_ORG/cdl-jupyter:$PYTHON_VERSION" \
...
build_custom() {
    ...
    source "$GITHUB_WORKSPACE/cdl-jupyter/ci/custom-args.sh"
    docker build --rm --force-rm \
        -f "$image_dir/Dockerfile" \
        -t "$DOCKER_HUB_ORG/cdl-jupyter:${PYTHON_VERSION}-custom" \
